...
def train(config, data, model, criterion, optim, epoch, output_dir,
    ...
    for i, (input, target, weight, meta) in enumerate(data):
        ...
        for t, w, o in zip(target, weight, output):
            t = t.cuda(2,non_blocking=True)
            w = w.cuda(2,non_blocking=True)
...
def validate(config,
    ...
    if config.DATASET.TEST_DATASET == 'multiview_h36m' or config.DATASET.TEST_DATASET == 'multiview_3dhp_2d' or config.DATASET.TEST_DATASET == 'multiview_3dhp':
        nviews = 4
