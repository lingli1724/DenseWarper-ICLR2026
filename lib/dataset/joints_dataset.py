...
class JointsDataset(Dataset):
    ...
    def __getitem__(self, idx, cen=None, sca=None, rot=None, source='h36m', **kwargs):
        db_rec = copy.deepcopy(self.db[idx])
        #print(db_rec['camera']['R'])
        #print(db_rec['camera']['T'])
        #print("-----------------------------------")
        image_dir = 'images.zip@' if self.data_format == 'zip' else ''
        image_file = osp.join(self.root, 'human36', image_dir, 'images',
                              db_rec['image'])
